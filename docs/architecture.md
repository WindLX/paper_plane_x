...
**核心原则**：
- **极简工程设计**：弃用 LangGraph/Celery 等重型图计算和任务调度框架，依托 FastAPI 后台任务与 SQLite 数据库维护系统状态机。
...
- **持久化层**：`SQLite` (存储元数据、项目状态机上下文、利用 FTS5 插件实现全文检索), `ChromaDB` (轻量级本地向量库，用于语义检索)
- **前端交互**：`Vue 3` + `Vite` + `TailwindCSS`
...
- `POST /api/v1/projects/{id}/data-process`（单文件上传，后台异步处理）
- `POST /api/v1/projects/{id}/data-process/{paper_id}/retry`（同 ID 重传重试）
- `GET /api/v1/projects/{id}/data-process/tasks`（任务状态总览）
- `POST /api/v1/projects/{id}/data-process/tasks/{task_id}/cancel`（终止任务）
- `POST /api/v1/projects/{id}/data-process/tasks/{task_id}/retry`（失败/取消任务重试）

接口返回 `202 Accepted` 后，处理在后台 data-process worker 池中完成。上传与重试接口返回 `task_id`，用于查询队列状态、终止任务与失败重试。
...
- `services/data_process_orchestrator.py`: Data Process 业务编排与领域错误边界
- `services/data_process_task_manager.py`: Data Process 任务队列、worker lifecycle、cancel/retry 状态流

针对单篇论文，采用**串行多阶段**处理机制：
...
##### 3.3.1.2 阶段二：信息精炼 (Data Extraction)
由 `ExtractionAgent` 负责，基于 LiteLLM 对全文内容进行结构化提炼。
- **Input**: `raw_md`, `images` (可选)
- **Long Memory**: `System.md` (设定专家角色), `Extraction.md` (设定提取指令与标准)。
- **Short Memory**: 若上一轮未通过事实核查，此轮将注入上一轮的 *产物* 及 *核查失败报告* 作为反思上下文。
- **Output (Pydantic 强制约束)**: `QuickScan` (速览级摘要信息), `SynthesisData` (深度综述级结构化数据)。
- **鲁棒性机制**：内置解析重试逻辑。若 LLM 输出无法被 Pydantic 解析，将自动重试；达最大重试次数则挂起任务通知人类。

##### 3.3.1.3 阶段三：事实核查 (Fact Checking)
由独立的 `FactCheckAgent` 负责，严格比对“提取阶段产物”与“原始解析产物”，防止 LLM 幻觉。
- **Input**: `raw_md` (作为 ground truth), 阶段二生成的 `QuickScan` 与 `SynthesisData`。
...
- **流程控制**：若 `is_passed == False`，系统会将 `correction_suggestions` 传回阶段二重新提炼，形成局部修正闭环。

##### 3.3.1.4 阶段四：数据入库 (Knowledge Ingestion)
将验证无误的最终结构化数据持久化：
- **SQLite 存储**：论文元数据、结构化提取字段（JSON 存入）、事实核查记录、原始 Markdown 文本（建立 FTS5 索引供关键词精准检索）。
- **ChromaDB 存储**：规划中（Phase 4），当前代码路径尚未接入生产写入。

##### 3.3.1.5 数据处理流伪代码示例
```python
# 伪代码：反映“上传入队 + 后台消费 + 局部闭环”
...
async def worker_process_task(paper_id, pdf_path):
    ...
    while retry_count < MAX_RETRY_COUNT:
        extracted_data = await extraction_agent.run(
            md_content=raw_md,
            images=image_paths,
            previous_feedback=previous_feedback,
        )
        fact_check = await fact_check_agent.run(raw_md, extracted_data, image_paths)
        if fact_check.is_passed:
            break
        previous_feedback = fact_check
        retry_count += 1

    if not fact_check.is_passed:
        update_status(paper_id, "FAILED")
        return

    save_results_to_sqlite(paper_id, raw_md, image_paths, extracted_data, fact_check)
```
...
- **`Librarian`** *(TODO)*: 文献检索工具合集（关键词查询、语义查询、原文拉取），供 Writer/Survey Agent 获取素材。
...
- `papers`: 论文元数据与处理产物（含 `md_content`、`raw_pdf_path`、`images_paths`、结构化提取和核查结果，以及 `final_fact_check_trace_id`）
- `agent_traces`: Agent 交互轨迹与 LLM 用量信息（`llm_model`、`prompt_tokens`、`completion_tokens`、`total_tokens`、`usage_payload`）
- `workflow_history`: 状态迁移历史（表名沿用）
